...
import type { PublicLiveSession } from "#/components/live/types";
...
type SocketContextType = {
  ...
  liveSessions: Record<string, PublicLiveSession>;
...
function SocketProvider({
  ...
  const [liveSessions, setLiveSessions] =
    React.useState<Record<string, PublicLiveSession>>(initialLiveSessions);
  ...
  React.useEffect(() => {
    ...
    const handleLiveStarted = (session: PublicLiveSession) => {
      setLiveSessions((prev) => ({ ...prev, [session.roomId]: session }));
    };
    const handleLiveEnded = ({ roomId }: { roomId: string }) => {
    ...
    newSocket.on("room.live.started", handleLiveStarted);
    newSocket.on("room.live.ended", handleLiveEnded);
